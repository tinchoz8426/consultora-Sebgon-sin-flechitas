...
{ 
$database->elimina("opciones" ,"IDEN=$MODIFICA");
...
if (isset($_POST['op_niv_1'])) 
{ 
IF (($IDEN=="") AND ($MODIFICA==""))
{
	$insertando=var_for();	
	$database->insert("opciones" ,"$insertando"); 
...
{
$insertando=var_for();
$database->update("opciones" ,"$insertando","IDEN=$MODIFICA");
...
else
{
	...
	if ($IDEN<>"")
	{
		$clientes_sql = "SELECT * from opciones where IDEN=".$IDEN ;
	...
	$formulario .= $frm->addInput("form-control ","text","NOMBRE","Nombre",$row['NOMBRE'],"col-md-8","required","norequired");
	$formulario .= $frm->addInput("form-control ","text","LINK","Link",$row['LINK'],"col-md-8","norequired");
	...
	$formulario .= $frm->addBoton('<button name="op_niv_1" type="submit" class="btn btn-primary">Confirma</button>');
	$formulario .= $frm->addBoton(' <button name="cancela" type="submit" class="btn btn-primary-black">Cancela</button>');
	$formulario .= $frm->addBoton(' <button name="elimina_op_niv_1" type="submit" class="btn btn-primary-black">Elimina</button>');
