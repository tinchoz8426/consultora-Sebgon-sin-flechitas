...
CREATE TABLE `users` (
  ...
  PRIMARY KEY  (`id`)
    ...
    <textarea name="sql" id="sql" cols="80" rows="10"><?php if (isset($_REQUEST['sql'])) echo stripslashes($_REQUEST['sql']); else echo '' ?></textarea>

  <p>Include File Name. You create this file. <a href="javascript:showHint('include_hint');">[Example]</a><br /> 
    <input name="include" type="text" id="include" value="<?php if (isset($_REQUEST['include'])) echo stripslashes($_REQUEST['include']); else echo 'openbase.php' ?>" />
    </p>
...
if (! mysql_select_db('foo') ) {
  ...
  <p>Primary Key Name<br /> 
    <input name="id_key" type="text" id="id_key" value="<?php if (isset($_REQUEST['id_key'])) echo stripslashes($_REQUEST['id_key']); else echo 'IDEN' ?>" />
    </p>
  <p>
    File Name of List<br /> 
    <input type="text" name="list_page" value="<?php if (isset($_REQUEST['list_page'])) echo stripslashes($_REQUEST['list_page']); else echo 'list.php' ?>" id="list_page" />
    </p>
  <p>File Name of New<br /> 
    <input type="text" name="new_page" value="<?php if (isset($_REQUEST['new_page'])) echo stripslashes($_REQUEST['new_page']); else echo 'new.php' ?>" id="new_page" />
    </p>
  <p>File Name of Edit<br />
    <input type="text" name="edit_page" value="<?php if (isset($_REQUEST['edit_page'])) echo stripslashes($_REQUEST['edit_page']); else echo 'edit.php' ?>" id="edit_page" />
</p>
  <p>File Name of Delete<br />
    <input type="text" name="delete_page" value="<?php if (isset($_REQUEST['delete_page'])) echo stripslashes($_REQUEST['delete_page']); else echo 'delete.php' ?>" id="delete_page" />
  </p> 
